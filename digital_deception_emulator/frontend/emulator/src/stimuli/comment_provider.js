...
export default class CommentProvider {
    ...
    generateShuffledBank() {
        ...
        let rootComments = this.comment_bank.availableComments(this.comment_bank.rootCategory);
        let numberRootPerCategory = Math.trunc(rootComments.length / (categories.length - 1));
        let currentRootIndex = 0;

        for (let i = 0; i < categories.length; i++) {
            let category = categories[i];
            let extraComments = rootComments.slice(currentRootIndex, currentRootIndex + numberRootPerCategory);
            ...
            let categoryComments = this.comment_bank.availableComments(category);
            ...
            this.shuffled_comment_bank[category] = { current_index: 0, comments: randomizedComments };
        }
    ...
    getNextComment(post) {
        ...
        if (!(category in this.shuffled_comment_bank)) {
            console.error("Could not find category " + category + " in shuffle comment bank, what happened??");
        }

        let useSpecificCategory = true;

        if (!useSpecificCategory) {
            console.debug("Using general category instead of specific this time");
            category = this.comment_bank.rootCategory;
        }

        console.debug("Looking for category in shuffled comment bank " + category);
        ...
        if (comment_index >= comments.length) {
            console.warn("We were going to use specific category " + category + " but that comment bank has run out, using root category instead");
            category = this.comment_bank.rootCategory;
        ...
        if (this.all_picked_comment_ids.includes(comment.body)) {
            console.warn("Picked comment " + comment.body + " has already been picked once before???");
        ...
        console.debug("Picked comment " + comment.body + " for post id " + post.id);
        console.debug("Comment bank current index before updating: " + this.shuffled_comment_bank[category].current_index);

        comment_data.current_index++;
        console.debug("Comment bank current index after updating: " + this.shuffled_comment_bank[category].current_index);
...
export class PracticeCommentProvider {
    constructor(practice_comment_bank, author_provider) {
        ...
        this.author_provider = author_provider;
        this.shuffled_comment_bank = shuffle(practice_comment_bank.availableComments());
        this.current_index = 0;
    }

    getNextComment(post) {
        let comment = this.shuffled_comment_bank[this.current_index % this.shuffled_comment_bank.length];
        comment.author = this.author_provider.getNextAuthor();
        this.current_index++;
        return comment;
    }
