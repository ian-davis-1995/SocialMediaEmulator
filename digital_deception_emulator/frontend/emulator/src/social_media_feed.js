...
export class SocialMediaFeed extends React.Component {
            ...
            <div className="quiz-container container-fluid">
                <div className="row">
                    <div className="col ml-3">
                        <button onClick={() => this.onQuizFinished()}>Back</button>
                    </div>
                </div>
                <div className="row">
    ...
    componentDidMount() {
        ...
        if (this.props.practiceMode) {
            this.userInteractionTracker.startTracking("practice-" + this.props.testId);
        } else {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (this.state.showingKnowledgeCheck || this.state.viewingReplies || this.state.experimentFinished) {
    ...
    render() {
        ...
        for (let id in this.state.posts) {
            if (this.state.posts.hasOwnProperty(id)) {
                ...
                let hasErrors = post.answer === null || post.answer === undefined;
                console.debug("Post has errors: " + hasErrors);
                console.debug("Showing validation errors: " + this.state.showingEmulatorValidation);
                ...
                items.push(
                    <SocialMediaPost
                        ...
                        modal={false}
                        hasErrors={hasErrors}
                        showValidationErrors={this.state.showingEmulatorValidation}
                        key={id}
                        onQuizClicked={() => this.setState({ showingQuiz: true })}
                        ...
                        onVerticalSeparatedStoryClicked={() => this.onVerticalSeparatedStoryClicked(id)}
                        onStorySegmentRead={this.onStorySegmentRead}
                        onMultiChoiceAnswerChanged={(postId, answer) => this.onMultiChoiceAnswerChanged(postId, answer)}
                        onPriorKnowledgeChanged={(postId, answer) => this.onPriorKnowledgeChanged(postId, answer)}
                    />);
            }
        }
        ...
        if (this.state.posts !== undefined && this.state.posts !== null && this.state.modalId !== 0) {
            currentAnswer = this.state.posts[this.state.modalId]["answer"];
            console.debug(`[SOCIAL_MEDIA_FEED][PRIOR_KNOWLEDGE] - Current Prior Knowledge question answer == ${this.state.modalId}`);
        }
        ...
        return (
            <div className={"main-container"} id={"mainEmulatorContainer"}>
                ...
                <ExperimentPlayback data={this.props.heatmapData}
                ...
                <ModalDialog
                    ...
                    onBackgroundClicked={() => { }}
                    onDialogClicked={(event) => this.onDialogClicked(this.state.postRepliesToView, event)}
                    body={
                        <PriorKnowledgeQuestion
                            id={this.state.modalId}
                            onPriorKnowledgeChanged={this.onPriorKnowledgeChanged}
                            onSubmitClicked={() => this.onPriorKnowledgeSubmitted(this.state.modalId, this.state.posts[this.state.modalId]["answer"])}
                ...
                <ModalDialog
                    ...
                    onDialogClicked={(event) => this.onDialogClicked(this.state.postRepliesToView, event)}
                    body={
                        <ExperimentFinishedDialog practiceMode={this.props.practiceMode} />
                    } />
                <ModalDialog id={`validation-dialog`}
                    visible={this.state.showingValidationDialog}
                    onDialogClicked={(event) => { event.stopPropagation() }}
                    onBackgroundClicked={() => { }} body={
                        <ValidationErrorDialog onDialogDismissed={this.onValidationDialogDismissed} />
                    } />
            </div>
        );
    }
    ...
    onWindowMessage(event) {
        ...
        if (event_type == "user_continued_survey") {
            if (this.state.experimentFinished) {
                console.debug("Ignoring validation message because experiment already finished");
                return;
            }
    ...
    onMultiChoiceAnswerChanged(postId, answer) {
        if (this.bottomValidationTimer != null) {
            console.debug("Cancelling idle validation timer since user answered distractor task");
            clearTimeout(this.bottomValidationTimer);
            this.bottomValidationTimer = null;
        }

        this.setState((state, _) => {
            this.userInteractionTracker.onMultiChoiceAnswerChanged(postId, answer);
    ...
    onPriorKnowledgeSubmitted(postId, answer) {
        ...
        console.debug("[SOCIAL_MEDIA_FEED][ON_PRIOR_KNOWLEDGE_SUBMITTED] - postId = " + postId + " answer = " + cleanedAnswer);
        let readTime = Date.now() - this.priorKnowledgeStartTime;
        this.userInteractionTracker.onPriorKnowledgeAnswered(Date.now(), postId, cleanedAnswer, readTime);
        this.priorKnowledgeStartTime = Date.now();
        this.setState({
            showingKnowledgeCheck: false,
        })
    ...
    onStorySegmentRead(timestamp, postId, segmentNumber, readTime) {
        this.userInteractionTracker.onStorySegmentRead(timestamp, postId, segmentNumber, readTime);
    }
    ...
    onRepliesClicked(id) {
        console.debug("[SOCIAL_MEDIA_FEED][EVENT][MOUSE] - View replies button clicked for " + id);
        this.setState({
            modalId: id,
            viewingReplies: true,
    ...
    showUserValidation() {
        ...
        if (this.state.viewingVerticalStory) {
            console.debug("Ignoring user idle scroll event because we are viewing a vertical story");
            return;
        } else if (this.state.showingKnowledgeCheck) {
            console.debug("Ignoring user idle scroll event because we are viewing knowledge check");
            return;
        } else if (this.state.showingValidationDialog) {
            console.debug("Ignoring user idle scroll event because we are already showing validation dialog");
            return;
        } else if (this.state.showingEmulatorValidation) {
            console.debug("Ignoring user idle scroll event because we have already shown emulator validation");
            return;
        } else if (this.state.experimentFinished) {
            console.debug("Ignoring user idle scroll event because experiment has finished");
            return;
        } else if (this.props.practiceMode) {
            console.debug("Ignoring user validation display because we are in practice mode");
            return;
        }

        if (this.validationLimitReached) {
            console.debug("Not showing user validation because validation limit reached");
            return;
        }
    ...
    onUserReachedBottom() {
        if (this.bottomValidationTimer == null) {
            this.bottomValidationTimer = setTimeout(this.showUserValidation, 15000);
        }
    }
