...
export default class SeparatedStory extends React.Component {
  ...
  getPriorKnowledgeQuestion() {
    return <FillerQuestion id={this.props.id} />;
  }
    ...
    if (
      ...
      for (let i = 0; i < this.props.comments.length; i++) {
        ...
        comments.push(
          <div
            ...
            id={`${this.props.id}-comment-${comment.id}-container`}
            key={i}
          >
            <div
              className={"comment-top-bar"}
              id={`${this.props.id}-comment-${comment.id}-top-bar`}
            >
              <div
                className={"comment-author-container"}
                id={`${this.props.id}-comment-${comment.id}-author-container`}
              >
                {comment.author}
              </div>
              <div
                className={"comment-date-container"}
                id={`${this.props.id}-comment-${comment.id}-date-container`}
              >
                {comment.postDate}
              </div>
            </div>
            <div
              ...
              id={`${this.props.id}-comment-${comment.id}-body`}
            >
              {comment.body}
            </div>
          </div>
      ...
      return (
        <div
          ...
          id={`${this.props.id}-comments-container`}
        >
          {comments}
        </div>
      );
  ...
  getStoryTexts() {
    if (this.state.currentChunkShown === this.props.storySegments.length) {
      ...
      this.state.currentChunkShown ===
      this.props.storySegments.length + 1
    ) {
  ...
  render() {
    return (
      <div
        ...
        className={"post-body-container separated-story-container"}
      >
        <div
          className={"story-chunk-container"}
          id={`${this.props.id}-chunk-container`}
        >
          <SwitchTransition>
            <CSSTransition
              key={this.state.currentChunkShown}
              timeout={250}
              classNames="story-chunk-text-animated"
            >
              <div
                className={"story-chunk-text"}
                id={`${this.props.id}-chunk-text`}
              >
                <span>{this.getStoryTexts()}</span>
              </div>
            </CSSTransition>
          </SwitchTransition>
        </div>
        <button
          id={`${this.props.id}-story-arrow`}
          className={"story-arrow"}
          onClick={this.onArrowClicked}
          disabled={
            this.state.currentChunkShown >= this.props.storySegments.length + 1
          }
        >
          Next
        </button>
