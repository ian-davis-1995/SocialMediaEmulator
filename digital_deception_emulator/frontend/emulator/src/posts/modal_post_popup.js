...
function ModalPostLayout(props) {
    ...
    if (props.post.profilePicture !== undefined) {
        profilePicture = (
            <div
                ...
                id={props.id + "-modal-profile-picture-container"}
            >
                <img
    ...
    return (
        <div id={props.id}>
            <div
                className={"post-title-container modal-post-title"}
                id={props.id + "-modal-post-title"}
            >
                {profilePicture}
                <div
                    className={"post-author-container"}
                    id={props.id + "-modal-post-author-container"}
                >
                    {props.post.author}
                </div>
                <div
                    className={"post-date-container"}
                    id={props.id + "-modal-post-date-container"}
                >
                    {props.post.postDate}
                </div>
            ...
            <hr
                className={"post-section-separator"}
                id={props.id + "-modal-post-section-separator"}
            />
            <div
                id={props.id + "-modal-post-body"}
            ...
            <div
                id={props.id + "-modal-replies-body"}
                className={"modal-replies-body"}
            >
                {repliesSection}
            </div>
        </div>
...
function RepliesSection(props) {
    ...
    return (
        <div
            className={"modal-replies-container"}
            id={props.id + "-replies-container"}
        >
            <hr
                className={"post-section-separator"}
                id={props.id + "-section-separator"}
            />
            <div
                className={"modal-replies-height-container"}
                id={props.id + "-replies-height-container"}
            >
                {items}
            </div>
        </div>
...
export function ReplyLayout(props) {
    ...
    if (props.reply.profilePicture !== undefined) {
        profilePictureContainer = (
            <div
                className={"modal-profile-picture-container"}
                id={props.reply.id + "-profile-picture-container"}
            >
                <img
                    src={props.reply.profilePicture}
    ...
    return (
        <div className={"modal-reply-container"} id={props.reply.id}>
            <div
                className={"modal-reply-flex-wrapper"}
                id={props.reply.id + "-flex-wrapper"}
            >
                {profilePictureContainer}
                <div
                    className={"modal-reply-body"}
                    id={props.reply.id + "-body"}
                >
                    <h5
                        className={"modal-reply-author"}
                        id={props.reply.id + "-author"}
                    >
                        {props.reply.author}
                    </h5>
                    {props.reply.body}
...
export default function ModalPostPopup(props) {
    return (
        <div
            ...
            className={
                "modal-dialog" +
                (props.visible ? " modal-visible" : " modal-hidden")
            }
