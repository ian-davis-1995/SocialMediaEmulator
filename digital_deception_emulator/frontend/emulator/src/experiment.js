...
class PilotExperiment extends React.Component {
  ...
  componentDidMount() {
    ...
    if (process.env.NODE_ENV !== "production" || debug) {
      ...
      console.debug(`posting emulator_info message with storyIds: ${storyIds}`);
      window.postMessage({
  ...
  render() {
    ...
    for (let i = 0; i < this.state.newsStoryIds.length; i++) {
      ...
      let distractorTaskId =
        this.state.distractorTaskIds[
          distractorTaskIndex % this.state.distractorTaskIds.length
        ];
      ...
      console.debug(
        `[FEED_GENERATION] - distractorTaskId = ${distractorTaskId} distractorTask = ${distractorTask} distractorTaskIndex = ${distractorTaskIndex}`
      );
      ...
      newsStory["comments"] =
        this.commentProvider.getCommentsForPost(newsStory);

      actualPosts.push(newsStory);
      actualPostIds.push(newsStoryId);

      if (
        i < this.state.newsStoryIds.length - 1 ||
        this.state.newsStoryIds.length === 1
      ) {
        if (distractorTask === undefined) {
          console.debug(
            "[FEED_GENERATION] - distractor task was undefined, skipping."
          );
          continue;
        }
    ...
    if (!practiceMode) {
      ...
      console.debug(
        `[FEED_GENERATION] - Adding distractor task number ${lastDistractorIndex} to have a distractor at the end of the experiment.`
      );

      let distractorId =
        this.state.distractorTaskIds[
          lastDistractorIndex % this.state.distractorTaskIds.length
        ];
      let distractorTask = this.props.posts[distractorId];

      console.debug(
        `[FEED_GENERATION] - last distractor task id = ${distractorId}`
      );
